...
def gen_set(pos_1_set, 
    ...
    WORDS = Counter(spell_checker.words(os.path.join('./Dict', "negative-words.txt")))
    ...
    for doc_id in doc_ids:
        ...
        filter_words = preprocess.process_tokens(terms_all, stop, punctuation, emojis, pos_emoti, neg_emoti, WORDS)
        count += 1
        if count % 10 == 0:
            print(count)
        if len(filter_words) > 1:
            ...
            count_positive, count_negative, total_tweet, polarity_tweet  = create_sentiment.create_training_set(filter_words,                                
                                                                                    total_tweet,
                                                                                    polarity_tweet,
                                                                                    count_positive,
                                                                                    count_negative)
            count_positive = 0
            count_negative = 0
    return total_tweet, polarity_tweet
